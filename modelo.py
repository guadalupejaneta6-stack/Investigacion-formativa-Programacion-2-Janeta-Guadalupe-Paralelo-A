...
def entrenar_modelos(df):
    ...
    # Una clase que represente la Media y la Mediana usando lógica de arreglos

class AnalizadorEstructuras:
    def __init__(self, datos):
        # Los datos se almacenan en una estructura de Arreglo (Array)
        self.datos = list(datos) 

    def calcular_media_manual(self):
        # Demostración de recorrido de estructuras lineales
        suma = 0
        for valor in self.datos:
            suma += valor
        return suma / len(self.datos)

    def obtener_mediana_ordenada(self):
        # Demostración de Algoritmos de Ordenamiento (Sorting)
        lista_ordenada = sorted(self.datos)
